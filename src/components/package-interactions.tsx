...
export function PackageInteractions({
  packageId,
  packageName,
  travelName,
  isVerified,
  ...
  const checkFavoriteStatus = async () => {
    ...
    if (isLoggedIn && user) {
      ...
      try {
        ...
        if (result.success) {
          setIsFavorite(result.data.includes(packageId))
        }
      } catch (error) {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (!isFavorite) {
        ...
        const response = await fetch(`/api/favorites?email=${encodeURIComponent(userData.email)}&packageId=${packageId}`, {
          method: 'DELETE'
        })
...
🏢 ${travelName}${isVerified ? ' ✅' : ''}

⏱️ Durasi: ${duration}
📅 Keberangkatan: ${formattedDate}
✈️ Kota Keberangkatan: ${departureCity}
💰 Harga Mulai: ${formatCurrency(price)}${cashback > 0 ? `\n🎁 Cashback: ${formatCurrency(cashback)}` : ''}
