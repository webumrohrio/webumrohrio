...
export default function CreatePackagePage() {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      // Get travel ID from session
      const travelSession = JSON.parse(localStorage.getItem('travelAdminSession') || '{}')
      ...
      const response = await fetch('/api/travel-admin/packages', {
        method: 'POST',
      ...
      if (result.success) {
        router.push('/travel-admin/packages?status=success&action=created')
      } else {
        const errorMessage = result.error || 'Gagal menambahkan paket'
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        message="Pastikan Data Sudah di Masukan Dengan Benar, Tanggal Keberangkatan Tidak Dapat Diubah Setelah Data Tersimpan.&#10;&#10;Apakah Anda yakin ingin menyimpan paket ini?"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                placeholder="Memuat kota..."
                required
              />
              <p className="text-xs text-gray-500 mt-1">Kota keberangkatan diambil dari profil travel Anda</p>
            </div>
            <div>
